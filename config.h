 ...
 */
static char *font = "Agave Nerd Font Mono:pixelsize=20:antialias=true:autohint=true";

/* Spare fonts */
static char *font2[] = {
	"monospace:pixelsize=20:antialias=true:autohint=true",
 ...
 * 2 last for foreground, background, cursor colors),
	...
	{  // Iceberg-Dark (1)
		{
			...
			"#6b7089",  // 8 - black (bright) / grey
			"#e98989",  // 9 - red (bright)
			"#c0ca8e",  // 10 - green (bright)
			"#e9b189",  // 11 - yellow (bright)
			"#91acd1",  // 12 - blue (bright)
			"#ada0d3",  // 13 - magenta (bright)
			"#95c4ce",  // 14 - cyan (bright)
			"#d2d4de",  // 15 - white (bright)

			// Extra colors (used for foreground, background, cursor)
			[256]="#c6c8d1",
			"#161821"
		},
	...
	{  // Iceberg-Light (2)
		{
			...
			"#8389a3",  // 8 - white (dimmed) / grey
			"#cc3768",  // 9 - red (dimmed)
			"#598030",  // 10 - green (dimmed)
			"#b6662d",  // 11 - yellow (dimmed)
			"#22478e",  // 12 - blue (dimmed)
			"#6845ad",  // 13 - magenta (dimmed)
			"#327698",  // 14 - cyan (dimmed)
			"#262a3f",  // 15 - black (dimmed)
	...
	{  // Kanagawa (3)
		{
			...
			"#727169",  // 8 - black (bright) / grey
			"#e82424",  // 9 - red (bright)
			"#98bb6c",  // 10 - green (bright)
			"#e6c384",  // 11 - yellow (bright)
			"#7fb4ca",  // 12 - blue (bright)
			"#938aa9",  // 13 - magenta (bright)
			"#7aa89f",  // 14 - cyan (bright)
			"#dcd7ba",  // 15 - white (bright)
...
#define ST_DEFAULT 0
#define ICEBERG_DARK 1
#define ICEBERG_LIGHT 2
#define KANAGAWA 3
...
/* Shortcut mask aliases/definitions */
 ...
 * (MouseShortcutRepeat)
 * repeat value:
 * * > 0: amount of times to execute function
 */
...
static MouseShortcutRepeat mshortcutsr[] = {
	/* mask             button          function        argument        release altscrn repeat */
	{ XK_ANY_MOD,       Button4,        ttysend,        {.s = "\031"},   0,      1,     mousescrollincrement },
	{ XK_ANY_MOD,       Button5,        ttysend,        {.s = "\005"},   0,      1,     mousescrollincrement },
};
 ...
 * (ShortcutIgnore)
 * ignore value:
 * * 0: shortcut is always active (don't care about "pname")
 * * -1: shortcut is only active when "pname" IS running as child of current terminal (exclusive for "pname")
 * * 1: shortcut is only active when "pname" IS NOT running as child of current terminal (ignored for "pname")
 * pname value:
 * * char[16]: name of targeted process
 */
static Shortcut shortcuts[] = {
	/* mask             keysym          function        argument    altscrn */
	...
	{ TERMMOD,          XK_C,           clipcopy,       {.i =  0},  -1 },
	{ TERMMOD,          XK_V,           clippaste,      {.i =  0},   0 },
	{ XK_NO_MOD,        XK_F11,         fullscreen,     {.i =  0},   0 },
	{ ShiftMask,        XK_Page_Up,     kscrollup,      {.i = -1},   0 },
	{ ShiftMask,        XK_Page_Down,   kscrolldown,    {.i = -1},   0 },
	{ TERMMOD,          XK_Return,      newterm,        {.i =  0},   0 },
	{ ControlMask,      XK_equal,       zoom,           {.f = +2},   0 },
	{ TERMMOD,          XK_plus,        zoom,           {.f = +2},   0 },
	{ ControlMask,      XK_minus,       zoom,           {.f = -2},   0 },
	{ TERMMOD,          XK_underscore,  zoom,           {.f = -2},   0 },
	{ ControlMask,      XK_0,           zoomreset,      {.f =  0},   0 },
};
static ShortcutIgnore shortcutsi[] = {
	/* mask             keysym          function        argument    altscrn ignore  pname */
	{ TERMMOD,          XK_C,           clipcopy,       {.i =  0},   1,      1,     "nvim" },
};
...
/* Altscreen */
static KeySym mappedkeys_altscrn[] = {
	XK_space,
	XK_m,
	XK_i,
	XK_A,
...
/* Altscreen */
static uint ignoremod_altscrn = Mod2Mask|XK_SWITCH_MOD;
...
static Key key_altscrn[] = {
	...
	{ XK_KP_1,          ControlMask,                    "\033[177;5u",   0,  0},
