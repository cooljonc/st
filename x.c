...
mouseaction(XEvent *e, uint release)
{
	...
	for (msr = mshortcutsr; msr < mshortcutsr + LEN(mshortcutsr); msr++) {
		if (msr->release == release &&
		    msr->button == e->xbutton.button &&
		    (!msr->altscrn || (msr->altscrn == (tisaltscr() ? 1 : -1))) &&
		    (match(msr->mod, state) ||  /* exact or forced */
		     match(msr->mod, state & ~forcemousemod))) {
			for(n = msr->repeat; n > 0; n--)
				msr->func(&(msr->arg));
			return 1;
		}
	}
		...
		if (base.mode & ATTR_UNDERLINE) {
			...
			if (base.ustyle != 3) {
				...
				XFillRectangle(xw.dpy, XftDrawDrawable(xw.draw), ugc, winx,
					winy + win.cyo + dc.font.ascent * chscale + 1, width, wlw);
...
match(uint mask, uint state)
{
	if (IS_SET(MODE_ALTSCREEN))
		return mask == XK_ANY_MOD || mask == (state & ~ignoremod_altscrn);
...
kmap(KeySym k, uint state)
{
	...
	if (IS_SET(MODE_ALTSCREEN)) {
		...
		for (i = 0; i < LEN(mappedkeys_altscrn); i++) {
			if (mappedkeys_altscrn[i] == k)
				break;
		}
		if (i == LEN(mappedkeys_altscrn)) {
			if ((k & 0xFFFF) < 0xFD00)
				return NULL;
		}
	
		for (kp = key_altscrn; kp < key_altscrn + LEN(key_altscrn); kp++) {
			if (kp->k != k)
				continue;
...
pid_t *
get_children(pid_t pid)
{
	char c;
	char children_path[PATH_MAX];
	int children_amount = 0;
	pid_t child_pid;
	pid_t *children = (pid_t *) malloc(sizeof(pid_t));

	snprintf(children_path, PATH_MAX, "/proc/%d/task/%d/children", pid, pid);
	FILE *stat = fopen(children_path, "r");
	if (stat == NULL) {
	...
	while (isdigit(c)) {
		ungetc(c, stat);
		fscanf(stat, "%d", &child_pid);
		c = fgetc(stat);

		children_amount++;
		children = (pid_t *) realloc(children, (children_amount + 1) * sizeof(pid_t));
		children[children_amount] = child_pid;
	...
	for (int i = 1; i <= children_amount; i++) {
		snprintf(children_path, PATH_MAX, "/proc/%d/task/%d/children", children[i], children[i]);
		FILE *stat = fopen(children_path, "r");
		if (stat == NULL) {
			fprintf(stderr, "couldn't open %s: %s\n", children_path, strerror(errno));
			continue;
	...
	children[0] = children_amount;
	return children;
}

bool
is_running(char pname[16])
{
	char c;
	char comm[16];
	...
	pid_t *children = get_children(currpid);

	for (int i = 1; i <= children[0]; i++) {
		snprintf(comm_path, PATH_MAX, "/proc/%d/comm", children[i]);
		FILE *stat = fopen(comm_path, "r");
		if (stat == NULL) {
...
kpress(XEvent *ev)
{
	...
	/*
	 * 2. shortcuts (shortcutsi)
	 *  - ignored when process with name shortcutsi->ignore is
	 *    running as child of the current terminal process
	 */
	for (bpi = shortcutsi; bpi < shortcutsi + LEN(shortcutsi); bpi++) {
		if (ksym == bpi->keysym && match(bpi->mod, e->state) &&
		    (!bpi->altscrn || (bpi->altscrn == (tisaltscr() ? 1 : -1))) &&
		    (!bpi->ignore || (bpi->ignore == (is_running(bpi->pname) ? -1 : 1)))) {
			bpi->func(&(bpi->arg));
			return;
		}
	}
